...
class ReviewService:
    """Service responsible for generating the HTML summary for the review screen."""
    
    def __init__(self):
        self.template_fields_loader = TemplateFieldsLoader()

    def generate_html_summary(self, data_model: ReportDataModel) -> str:
        """Generate HTML summary of all collected data."""
        if not data_model:
            return "Nenhum dado disponível para revisão."
        ...
        if data_model.template_path:
            summary_lines.append(f"<b>Template:</b> {data_model.template_path}")
        else:
        ...
        patient = data_model.patient_data
        ...
        if data_model.test_results:
            for test_name, test_data in data_model.test_results.items():
        ...
        if data_model.conclusion_text:
            conclusion_preview = data_model.conclusion_text[:200]
            if len(data_model.conclusion_text) > 200:
                conclusion_preview += "..."
        ...
        template_values = data_model.get_template_field_values()
        if template_values:
        ...
        field_mapping = data_model.get_field_mapping()
        if field_mapping:
